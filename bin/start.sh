...
startService() {
        ...
        source /root/.bashrc
        node main.min.js --debug-output=true --http.server.port=$2 --src-dirs=lib --main-class={COMPANY}.{PROJECT}.backend.Main --config-files=config/service.conf  --easynode.app.id=$1
}
